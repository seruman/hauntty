...
func TestReserveSessionName_GeneratesUnique(t *testing.T) {
	...
	name1, err := srv.reserveSessionName("")
	assert.NilError(t, err)
	assert.Assert(t, name1 != "")

	name2, err := srv.reserveSessionName("")
	assert.NilError(t, err)
	assert.Assert(t, name2 != "")
}
...
func TestPruneDeadSessions(t *testing.T) {
	...
	assert.NilError(t, err)
	var expected []string
	assert.DeepEqual(t, remaining, expected)
}
