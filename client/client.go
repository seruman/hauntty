...
// Create sends a headless session creation request (for ht new).
...
// Attach sends an attach request that creates-or-joins a session and
// transitions the connection to streaming mode on success.
...
// Kick removes a specific attached client from a session.
...
// Detach sends a streaming self-detach message.
